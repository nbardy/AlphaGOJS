...
| **TF.js readback** | Hot path cost is **GPU → CPU sync** (`dataSync` / gathers). Mitigations: **`src/engine/gpu_game_engine.js`** partial readbacks; **worker policy path** (`gpu_owner_runtime.js`) batched **forward + `multinomial` + `logSoftmax` gather** so rollout syncs **actions + log π + V** per row, not full **`[batch, boardSize]`** logits (PPO/REINFORCE/PPG). Training still uses full forward+backward in TF.js. |
