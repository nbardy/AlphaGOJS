...
| **TF.js WebGPU upload / Safari** | Stock **TF.js WebGPU** used **`mappedAtCreation`** for CPU→GPU tensor uploads; **WebKit** caps that path, so **large training tensors** throw **`createBuffer` … too large … mappedAtCreation** and can **kill the GPU worker** (canvases / instances die in cascade). **In-repo fix:** **`patch-package`** on **`@tensorflow/tfjs-backend-webgpu`** — **`queue.writeBuffer`** in **`uploadToGPU`** (`patches/@tensorflow+tfjs-backend-webgpu+4.22.0.patch`). Details: [key_learnings.md §7](./key_learnings.md). |
