...
- **Bench modes:** `bench:loop` with `sim_random` / `sim_forward` / full RL; **instrumentation** defaults **on** in **`bench:loop`** and **`bench:system:headless`** (sets `benchInstrument=1` unless overridden); **minimal UI** and other flags via URL — **no hot-path cost when absent** (`benchmarks/loop_decomposition_benchmark.mjs`, `benchmarks/system_interface_benchmark.mjs`, `gpu_owner_runtime.js`). User-facing matrix: **`docs/BENCHMARKS.md`**; dense semantics: **`agent_notes/key_learnings.md` §8**.
...
- **2026-04-04 (WebGPU train crash):** Safari/WebKit rejects large **`createBuffer({ mappedAtCreation: true })`** uploads in **TF.js WebGPU** `uploadToGPU`, breaking **PPO-scale** tensors in the **GPU worker** (`createBuffer failed, size (…) is too large … mappedAtCreation == true`). **Fix:** extend existing **`patch-package`** patch for **`@tensorflow/tfjs-backend-webgpu@4.22.0`** — **`queue.writeBuffer`** into a normal **`STORAGE`** buffer (`patches/@tensorflow+tfjs-backend-webgpu+4.22.0.patch`, **`dist/backend_webgpu.js`**). Documented in **`key_learnings.md` §7** and pitfall bullet in **§4**; **`readSync`** warnings and SES lockdown messages noted as **usually unrelated** to that throw.
- **2026-04-04 (benchmarks / docs):** Added **`docs/BENCHMARKS.md`** (script ↔ file ↔ layer matrix; tick vs inference terminology). **`system_interface_benchmark`**: default **`--instrument`**, **`trainerStatsForBench`** preserves worker timing fields in JSON; **`systemBenchLog`** respects **`--quiet`**; **`emitBenchmarkReport`**: **`--printJson`** works with **`--quiet`**. **`loop_decomposition_benchmark`**: **`generationsPerMinMedian`** in JSON + **`gen/min`** on summary lines. Root **`AGENTS.md`** benchmark section aligned (smoke vs full bundle, **`bench:matrix:smoke`**, instrument defaults). Captured in **`key_learnings.md` §8** and this line.
