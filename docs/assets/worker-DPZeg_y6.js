var P=`// alphagojs_v2/src/fused_ppo.wgsl
// --------------------------------------------------------------------------
...
`;function x(l){return P.replace(/const D: u32 = \d+u;/,`const D: u32 = ${l}u;`)}function W(l){const t=9*l,f=0+9*l*l+9*l*t+2*l*l+l+l+1,m=0+4*l+262144*l;return{D:l,K:24,P:8,PATCH_CH:t,denseWeightCount:f,embedWeightCount:m}}const E=[{name:"compact",D:4,color:"#44ff44"},{name:"standard",D:8,color:"#4488ff"},{name:"wide",D:16,color:"#ff4444"}],C="AlphaGoJS_v2_DB",b="checkpoints",A=1;class D{db=null;async init(){return new Promise((e,n)=>{const t=indexedDB.open(C,A);t.onupgradeneeded=a=>{const i=a.target.result;i.objectStoreNames.contains(b)||i.createObjectStore(b,{keyPath:"id"})},t.onsuccess=a=>{this.db=a.target.result,e()},t.onerror=a=>{n(a.target.error)}})}async saveCheckpoint(e){if(this.db)return new Promise((n,t)=>{const r=this.db.transaction([b],"readwrite").objectStore(b).put({id:e.id,embed:e.embed,dense:e.dense,elo:e.elo,generation:e.generation});r.onsuccess=()=>n(),r.onerror=u=>t(u.target.error)})}async deleteCheckpoint(e){if(this.db)return new Promise((n,t)=>{const r=this.db.transaction([b],"readwrite").objectStore(b).delete(e);r.onsuccess=()=>n(),r.onerror=u=>t(u.target.error)})}async loadAllCheckpoints(){return this.db?new Promise((e,n)=>{const i=this.db.transaction([b],"readonly").objectStore(b).getAll();i.onsuccess=r=>{const u=r.target.result;u.sort((h,f)=>h.id-f.id),e(u)},i.onerror=r=>{n(r.target.error)}}):[]}}class S{checkpoints=[];currentElo=1e3;eloK=32;nextId=1;storage=new D;async init(){await this.storage.init();const e=await this.storage.loadAllCheckpoints();if(e&&e.length>0){this.checkpoints=e;const n=e[e.length-1];n&&(this.currentElo=n.elo,this.nextId=n.id+1,console.log(`Loaded ${e.length} checkpoints from IDB. Current Elo: ${this.currentElo}`))}}async saveCheckpoint(e,n,t){const a={id:this.nextId++,embed:new Float16Array(e),dense:new Float32Array(n),elo:this.currentElo,generation:t};if(this.checkpoints.push(a),await this.storage.saveCheckpoint(a),this.checkpoints.length>20){const i=this.checkpoints.shift();i&&await this.storage.deleteCheckpoint(i.id)}}sampleOpponent(){if(this.checkpoints.length===0)return null;const e=Math.random(),n=Math.floor(Math.pow(e,.5)*this.checkpoints.length),t=Math.min(n,this.checkpoints.length-1);return this.checkpoints[t]}updateElo(e,n,t){const a=1/(1+Math.pow(10,(e-this.currentElo)/400)),i=t?.5:n?1:0;return this.currentElo+=this.eloK*(i-a),this.currentElo}}const s={numBoards:256,maxSteps:600,ppoEpochs:3,lr:3e-4,beta1:.9,beta2:.999,epsAdam:1e-8,epsilonClip:.2,c1Value:.5,c2Entropy:.01,gamma:.99,gaeLambda:.95,gradClip:1,weightDecay:.01,seed:42,wallDensity:.15,territoryBonus:.5,checkpointInterval:20,evalFraction:.5},B=24,G=B*B,g=Math.ceil(G/16),v=160+600*160,k=96;class T{device;pool=new S;archConfig;layout;paramsBuf;boardsBuf;embedBuf;denseBuf;readbackBuf;initPipeline;rolloutPipeline;gaePipeline;ppoPipeline;bindGroup;gaeBindGroup;ppoBindGroup;initBindGroup;adamStep=0;rollout=0;onStats;onBoard;constructor(e){this.archConfig=e,this.layout=W(e.D)}async init(){const e=await navigator.gpu.requestAdapter();if(!e)throw new Error("No GPU adapter");if(!e.features.has("shader-f16"))throw new Error("Your browser does not support the 'shader-f16' WebGPU feature. Please enable it in your browser flags (e.g. chrome://flags/#enable-webgpu-developer-features).");this.device=await e.requestDevice({requiredFeatures:["shader-f16"]}),await this.pool.init();const t=x(this.archConfig.D),a=this.device.createShaderModule({code:t,label:`plague_ppo_D${this.archConfig.D}`});console.log("Worker: Compiling initPipeline..."),this.initPipeline=await this.device.createComputePipelineAsync({layout:"auto",compute:{module:a,entryPoint:"init_boards"}}),console.log("Worker: Compiling rolloutPipeline..."),this.rolloutPipeline=await this.device.createComputePipelineAsync({layout:"auto",compute:{module:a,entryPoint:"rollout_step"}}),console.log("Worker: Compiling gaePipeline..."),this.gaePipeline=await this.device.createComputePipelineAsync({layout:"auto",compute:{module:a,entryPoint:"gae_scan"}}),console.log("Worker: Compiling ppoPipeline (this might take a few seconds)..."),this.ppoPipeline=await this.device.createComputePipelineAsync({layout:"auto",compute:{module:a,entryPoint:"ppo_step"}}),console.log("Worker: All pipelines compiled successfully!");const i=GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST,r=GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST,u=GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST,h=s.numBoards;this.paramsBuf=this.device.createBuffer({size:k,usage:r}),this.boardsBuf=this.device.createBuffer({size:h*v,usage:i}),this.embedBuf=this.device.createBuffer({size:this.layout.embedWeightCount*6*2,usage:i}),this.denseBuf=this.device.createBuffer({size:this.layout.denseWeightCount*6*4,usage:i}),this.readbackBuf=this.device.createBuffer({size:h*Math.max(v,4),usage:u});const f=new Float32Array(this.layout.denseWeightCount*4);let p=typeof Float16Array<"u"?new Float16Array(this.layout.embedWeightCount*4):null;if(this.pool.checkpoints.length>0){const c=this.pool.checkpoints[this.pool.checkpoints.length-1];console.log(`Worker: Resuming from IDB checkpoint ${c.id} at rollout ${c.generation}`),this.rollout=c.generation,f.set(c.dense,0),p&&c.embed&&p.set(c.embed,0)}else{let c=12345;const m=()=>(c=c*747796405+2891336453>>>0,c/4294967296);for(let o=0;o<this.layout.denseWeightCount;o++){const d=m(),_=m();f[o]=Math.sqrt(-2*Math.log(d+1e-12))*Math.cos(2*Math.PI*_)*.1}if(p)for(let o=0;o<this.layout.embedWeightCount;o++)p[o]=(m()-.5)*.02}this.device.queue.writeBuffer(this.denseBuf,0,f),p&&this.device.queue.writeBuffer(this.embedBuf,0,p),this.initBindGroup=this.device.createBindGroup({layout:this.initPipeline.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramsBuf}},{binding:1,resource:{buffer:this.boardsBuf}}]}),this.bindGroup=this.device.createBindGroup({layout:this.rolloutPipeline.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramsBuf}},{binding:1,resource:{buffer:this.boardsBuf}},{binding:2,resource:{buffer:this.embedBuf}},{binding:3,resource:{buffer:this.denseBuf}}]}),this.gaeBindGroup=this.device.createBindGroup({layout:this.gaePipeline.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramsBuf}},{binding:1,resource:{buffer:this.boardsBuf}}]}),this.ppoBindGroup=this.device.createBindGroup({layout:this.ppoPipeline.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.paramsBuf}},{binding:1,resource:{buffer:this.boardsBuf}},{binding:2,resource:{buffer:this.embedBuf}},{binding:3,resource:{buffer:this.denseBuf}}]})}writeParams(e,n,t){const a=new ArrayBuffer(k),i=new Uint32Array(a),r=new Float32Array(a);i[0]=s.numBoards,i[1]=e,i[2]=s.seed+this.rollout,i[3]=s.maxSteps,i[4]=this.adamStep,i[5]=n?1:0,r[8]=s.lr,r[9]=s.beta1,r[10]=s.beta2,r[11]=s.epsAdam,r[12]=s.epsilonClip,r[13]=s.c1Value,r[14]=s.c2Entropy,r[15]=s.gamma,r[16]=s.gaeLambda,r[17]=s.gradClip,r[18]=s.weightDecay,r[19]=t,r[20]=s.territoryBonus,r[21]=s.wallDensity,this.device.queue.writeBuffer(this.paramsBuf,0,a)}initBoards(){const e=this.device.createCommandEncoder(),n=e.beginComputePass();n.setPipeline(this.initPipeline),n.setBindGroup(0,this.initBindGroup),n.dispatchWorkgroups(s.numBoards),n.end(),this.device.queue.submit([e.finish()])}async runStep(){const e=performance.now();let n=!1,t=null,a=-1;this.rollout>0&&this.rollout%s.checkpointInterval===0&&await this.saveCheckpoint(),Math.random()<s.evalFraction&&(t=this.pool.sampleOpponent(),t&&(n=!0,this.device.queue.writeBuffer(this.denseBuf,this.layout.denseWeightCount*3*4,t.dense.buffer),this.device.queue.writeBuffer(this.embedBuf,this.layout.embedWeightCount*3*2,t.embed.buffer)));let i=1;n&&t&&(i=1/(1/(1+Math.pow(10,(t.elo-this.pool.currentElo)/400))+.1)),this.writeParams(0,n,i),this.initBoards();for(let o=0;o<s.maxSteps;o++){this.writeParams(o,n,i);const d=this.device.createCommandEncoder(),_=d.beginComputePass();if(_.setPipeline(this.rolloutPipeline),_.setBindGroup(0,this.bindGroup),_.dispatchWorkgroups(s.numBoards),_.end(),this.device.queue.submit([d.finish()]),o%20===0&&await this.allGamesDone())break}if(n&&t){const o=await this.getWinRate();a=o;const d=o===.5,_=o>.5;this.pool.updateElo(t.elo,_,d)}await this.captureBoard(),this.writeParams(0,n,i);{const o=this.device.createCommandEncoder(),d=o.beginComputePass();d.setPipeline(this.gaePipeline),d.setBindGroup(0,this.gaeBindGroup),d.dispatchWorkgroups(Math.ceil(s.numBoards/64)),d.end(),this.device.queue.submit([o.finish()])}for(let o=0;o<s.ppoEpochs;o++){this.writeParams(0,n,i);const d=this.device.createCommandEncoder(),_=d.beginComputePass();_.setPipeline(this.ppoPipeline),_.setBindGroup(0,this.ppoBindGroup),_.dispatchWorkgroups(s.numBoards),_.end(),this.device.queue.submit([d.finish()])}await this.device.queue.onSubmittedWorkDone();const u=(await this.readStepCounts()).reduce((o,d)=>o+d,0);this.adamStep+=Math.round(u/s.numBoards);const h=u/s.numBoards,f=await this.readLoss(),p=performance.now()-e,c=s.numBoards/p*1e3,m=p>0?u/p*1e3:0;this.rollout++,this.onStats&&this.onStats({rollout:this.rollout,games:this.rollout*s.numBoards,loss:f,elo:this.pool.currentElo,timeMs:p,trainedGamesPerSec:c,trainedStepsPerSec:m,avgStepsPerGame:h,winRate:a,entropy:0})}async allGamesDone(){const e=this.device.createCommandEncoder();for(let a=0;a<s.numBoards;a++)e.copyBufferToBuffer(this.boardsBuf,a*v+144,this.readbackBuf,a*4,4);this.device.queue.submit([e.finish()]),await this.readbackBuf.mapAsync(GPUMapMode.READ);const t=new Uint32Array(this.readbackBuf.getMappedRange().slice(0,s.numBoards*4)).every(a=>a!==0);return this.readbackBuf.unmap(),t}async getWinRate(){const e=this.device.createCommandEncoder();for(let r=0;r<s.numBoards;r++)e.copyBufferToBuffer(this.boardsBuf,r*v,this.readbackBuf,r*g*4,g*4);this.device.queue.submit([e.finish()]),await this.readbackBuf.mapAsync(GPUMapMode.READ);const n=new Uint32Array(this.readbackBuf.getMappedRange().slice(0,s.numBoards*g*4));let t=0,a=0;for(let r=0;r<s.numBoards;r++){let u=0,h=0;for(let f=0;f<g;f++){let p=n[r*g+f];for(let c=0;c<16;c++){const m=p>>c*2&3;m===1?u++:m===2&&h++}}u>h?t++:h>u&&a++}this.readbackBuf.unmap();const i=t+a;return i>0?t/i:.5}async captureBoard(){if(!this.onBoard)return;const e=this.device.createCommandEncoder();e.copyBufferToBuffer(this.boardsBuf,0,this.readbackBuf,0,g*4),this.device.queue.submit([e.finish()]),await this.readbackBuf.mapAsync(GPUMapMode.READ);const n=new Uint32Array(this.readbackBuf.getMappedRange().slice(0,g*4)),t=new Uint32Array(n);this.readbackBuf.unmap(),this.rollout%10===0&&console.log("captureBoard packed:",t[0],t[1]),this.onBoard(t)}async readLoss(){const e=this.device.createCommandEncoder();for(let t=0;t<s.numBoards;t++)e.copyBufferToBuffer(this.boardsBuf,t*v+160+8,this.readbackBuf,t*4,4);this.device.queue.submit([e.finish()]),await this.readbackBuf.mapAsync(GPUMapMode.READ);let n=0;if(typeof Float16Array<"u"){const t=new Float16Array(this.readbackBuf.getMappedRange().slice(0,s.numBoards*4));for(let a=0;a<s.numBoards;a++)n+=t[a*2]}return this.readbackBuf.unmap(),n/s.numBoards}async readStepCounts(){const e=this.device.createCommandEncoder();for(let a=0;a<s.numBoards;a++)e.copyBufferToBuffer(this.boardsBuf,a*v+152,this.readbackBuf,a*4,4);this.device.queue.submit([e.finish()]),await this.readbackBuf.mapAsync(GPUMapMode.READ);const n=new Uint32Array(this.readbackBuf.getMappedRange().slice(0,s.numBoards*4)),t=new Uint32Array(n);return this.readbackBuf.unmap(),t}async readWeights(){const e=this.layout.denseWeightCount*4,n=this.layout.embedWeightCount*2,t=e+n,a=this.device.createCommandEncoder();a.copyBufferToBuffer(this.denseBuf,0,this.readbackBuf,0,e),a.copyBufferToBuffer(this.embedBuf,0,this.readbackBuf,e,n),this.device.queue.submit([a.finish()]),await this.readbackBuf.mapAsync(GPUMapMode.READ);const i=this.readbackBuf.getMappedRange(0,t),r=new Float32Array(new Float32Array(i,0,this.layout.denseWeightCount)),u=new Float16Array(new Float16Array(i,e,this.layout.embedWeightCount));return this.readbackBuf.unmap(),{dense:r,embed:u}}async saveCheckpoint(){const e=this.layout.denseWeightCount*4,n=this.layout.embedWeightCount*2,t=e+n,a=this.device.createCommandEncoder();a.copyBufferToBuffer(this.denseBuf,0,this.readbackBuf,0,e),a.copyBufferToBuffer(this.embedBuf,0,this.readbackBuf,e,n),this.device.queue.submit([a.finish()]),await this.readbackBuf.mapAsync(GPUMapMode.READ);const i=this.readbackBuf.getMappedRange(0,t),r=new Float32Array(new Float32Array(i,0,this.layout.denseWeightCount));let u=new Float16Array(this.layout.embedWeightCount);typeof Float16Array<"u"&&(u=new Float16Array(new Float16Array(i,e,this.layout.embedWeightCount))),this.readbackBuf.unmap(),await this.pool.saveCheckpoint(u,r,this.rollout)}}let w,y=!1;self.onmessage=async l=>{if(l.data.type==="START"){self.postMessage({type:"LOG",msg:"Worker received START"});try{const e=l.data.archConfig??E[1];for(w=new T(e),w.onStats=n=>self.postMessage({type:"STATS",stats:n}),w.onBoard=n=>self.postMessage({type:"BOARD",board:n}),self.postMessage({type:"LOG",msg:"Initializing trainer..."}),await w.init(),self.postMessage({type:"LOG",msg:"GPUTrainer initialized"});;){if(y){await new Promise(n=>setTimeout(n,100));continue}await w.runStep()}}catch(e){self.postMessage({type:"ERROR",message:e.message||String(e)})}}else l.data.type==="PAUSE"?(y=!0,self.postMessage({type:"LOG",msg:"Training paused"})):l.data.type==="RESUME"&&(y=!1,self.postMessage({type:"LOG",msg:"Training resumed"}))};
//# sourceMappingURL=worker-DPZeg_y6.js.map
